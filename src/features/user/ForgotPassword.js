...
import ForgotPasswordOtpModal from "../../components/Forgot Password/ForgotPasswordOtpModal";
...
function ForgotPassword() {
  ...
  const [isForgotPasswordOtpModal, setIsForgotPasswordOtpModal] =
    useState(false);

  const submitForm = (e) => {
    e.preventDefault();
    setErrorMessage("");
    console.log("User Email: ", userObj.emailId);
    ...
    if (userObj.emailId.trim() === "")
      ...
      try {
        const postUserEmail = async () => {
          ...
          if (response.ok) {
            // console.log(
            //   "Post Response For Sending User Email to Backend: ",
            //   result
            // );
            setIsForgotPasswordOtpModal(true);
          }
        };
        postUserEmail();
      } catch (err) {
  ...
  return (
    <>
      ...
      {isForgotPasswordOtpModal && (
        <>
          <ForgotPasswordOtpModal email={userObj.emailId} />
        </>
      )}
    </>
  );
